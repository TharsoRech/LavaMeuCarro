...
public class ServicoRepository : IServicoRepository
{
    ...
    public async Task<List<Servico>> GetPromotionsAsync(int limit = 10) { using var db = _factory.CreateConnection(); return (await db.QueryAsync<Servico>("SELECT TOP(@Limit) s.* FROM Servicos s INNER JOIN Unidades u ON s.UnidadeId = u.Id WHERE s.Active = 1 AND s.IsPromotion = 1 AND (s.PromoEndDate IS NULL OR s.PromoEndDate > GETUTCDATE()) AND u.Ativo = 1 AND u.Published = 1 ORDER BY NEWID()", new { Limit = limit })).ToList(); }
